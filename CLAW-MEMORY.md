...
| 版本 | 核心机制 | 特点 |
|------|---------|------|
| V1 | SimHash + SQLite | 极小存储，三层归档 |
| V2 | V1 + Parent_ID链表 + 锚点 | 结构连贯，防迷失 |
| **V3** | **热度钉扎滑动窗** | **极简算力，越用越聪明** |

V3 不是对 V2 的补丁，而是**全面简化重写**：用单张 4 字段表取代 V2 的复杂 SimHash 体系，核心代码从 400 行缩减到不足 100 行。防迷失锚点机制从 V2 继承保留。
...
将 Claw 的记忆体系从"靠系统压缩"升级为"主动管理热度记忆"，在极小存储/算力约束下实现：

- **无限历史**：原文永久归档到 Markdown，存储不膨胀
- **瞬时召回**：关键词初筛 + 热度排序，毫秒级响应
- **越用越聪明**：有用的记忆热度飙升自动霸榜，无用的自然淘汰
